...
float att=50, med = 50, loAl, hiAl, loBe, hiBe, loGam, hiGam, thet, delt, s=100;
...
void setup() {
  ...
  trees.add(new Node(20, 7, random(20, width/2), height, -PI/2, 200));
  trees.add(new Node(36, 7, random(20, width/2), height, -PI/2, 200));

  flowers.add(new Flower(randomNode(), int(random(0, 200))));
  flowers.add(new Flower(randomNode(), int(random(0, 200))));
  ...
  testFlower = new Flower(randomNode(), int(random(0, 300)));
}
...
Node randomNode() {
  int treesGrass = int(random(0, 2))-1;
  Node returner;
  if (treesGrass == 0) {
    int whichTree = int(random(0, trees.size()-1.001));
    Node t = (Node) trees.get(whichTree);
    returner = t.find();
  } else {
    int whichBlade = int(random(0, grasses.length));
    returner = grasses[whichBlade].find();
  }
  return returner;
}
...
void watch() {
  if (s>50) {
    if (inUse == false) {
      inUse = true;
      trees.add(new Node(10, 7, random(10, width/3), height, -PI/2, 200));
    }
    counter = 600;
  } else {
    counter--;
  ...
  if (medEase>50) {
    ...
    fFlower f = (Flower) flowers.get(flowers.size()-1);
    if(millis()%3 == 0){
      if(f.s<4){
        f.s ++;
        //println("scale is now " + f.s);
      }else{
        flowers.add(new Flower(randomNode(), int(random(0, 200))));
      }
    }
...
void mouseClicked() {
  Flower f = (Flower) flowers.get(flowers.size()-1);
  f.s ++;
  //println(f.s);
}
...
void blow() {
  wind.x = (100-medEase)*oscillator/500;
